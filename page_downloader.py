...
class AiSpider(object):
    ...
    def request(self, _args, dont_filter):
        ...
        cookies = _args.get('cookies') if _args.get('cookies') else self.cookies
            ...
            try:
                if use_proxy:
                    ...
                    if ip:
                        ...
                        if method == 'get':
                            if cookies:
                                con = requests.get(url, headers=header, proxies=proxy, timeout=time_out, params=post_data, verify=False,cookies = cookies)
                            else:
                    ...
                    if method == 'get':
                        if cookies:
                            con = requests.get(url, headers=header, timeout=time_out, cookies=cookies)
                            print con
                        else:
